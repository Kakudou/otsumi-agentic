...
description: "Configure an Obsidian vault's graph view (graph.json) — color groups, search lens, display, and forces — by resolving the vault path from system.md instead of hardcoding it."
...
- MUST resolve the vault root from `system.md` → `## Knowledge Bases` → `{vault-id}` → `Absolute path`. NEVER hardcode a vault path, vault id, or folder layout in this skill or in any generated suggestion.
- MUST refuse with an explicit error if `system.md` has no `## Knowledge Bases` entry matching `{vault-id}`, or — when no id was given — if the section does not contain exactly one default vault marked `(default)`.
...
- MUST consult the vault's templates folder (per `system.md` → `Conventions source of truth`) before proposing queries targeting frontmatter fields, tag namespaces, or note types — field names MUST come from real templates, NEVER guessed.
...
1. If the user has no concrete grouping in mind:
   1. Read the vault's templates folder (declared on the vault entry under `Conventions source of truth`) to learn the canonical frontmatter keys, tag namespaces, and `Template:` values actually in use.
...
2. If no concrete query was supplied:
   1. Read the templates folder first to learn the frontmatter axes that real notes actually expose (e.g. `Template`, `Lang`, `Tags`, `#definition`).
...
- [ ] `system.md` resolved a vault entry; nothing was hardcoded in this skill or in the diff.
