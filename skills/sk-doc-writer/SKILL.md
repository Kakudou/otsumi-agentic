...
- NEVER write placeholder sections: every section MUST contain real content or be omitted entirely.
- MUST use **domain language first** in README/blog/changelog/Obsidian outputs.
- API reference MUST use interface terms (command, parameter, return value, endpoint) only when documenting a public surface.
- MUST lead with examples: show what it does before explaining how.
- MUST use **wikilinks** in Obsidian notes: `[[ADR-XXXX-title]]`, NEVER bare paths.
- MUST document trap and edge-case handling from Stage-2 when available — constraints are features.
- MUST add a footer to every document written: `**Written by the Hand of <persona name fallback to model name> and the Eyes of <user name fallback to 'the Dev'>.**`
...
1. Confirm `feature_name` is provided; STOP and report error if missing.
...
6. Determine documentation types: use `doc_types` if provided, otherwise default to `[readme, changelog]`.
7. Write the documentation files, applying all Hard Rules.
...
Multiple types can be produced in one pass.
...
A narrative explanation of the feature for a technical audience unfamiliar with the codebase.

### API reference

For features that expose a public interface: function, command, endpoint, or public contract.
