...
| `FUHYO_COMMIT_WITHOUT_GIT_SKILL` | error | If `atomic_task` (case-insensitive) contains commit-related phrases (`commit`, `git add`, `stage and commit`, `create a commit`, `atomic commit`) AND `authorized_skills` does not include `git-commits`, the step will bypass the commit skill's discipline (focused diffs, gitmoji, no push). The fix is to add `"git-commits"` to `authorized_skills`. |
