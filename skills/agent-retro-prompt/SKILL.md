...
Run a retrospective and produce a better starting prompt grounded in what actually happened.
...
- MUST ground every friction point in an actual event — NEVER invent friction.
- MUST keep the improved prompt within delivered scope — NEVER expand to undelivered work.
- MUST write the improved prompt as plain natural language — no commands, flags, stage numbers, or tool-specific syntax.
- Zero friction points is valid — report it honestly when the work ran clean.
- MAY be invoked at any time, not only after pipeline closure.
...
2. Reconstruct what was actually delivered:
   ...
   - scope suppressed, escalated, or added
   - corrections or rework and what triggered them
3. Identify friction points. Each MUST map to an actual event:
   - missing edge case
   - missing context
   - unstated constraint
   - scope ambiguity
