...
- Skills must stay portable: do not reference a specific agent persona, project mascot, or private orchestration name.
...
| `quality` | Generic validation, threshold scoring, and quality verdicts. |

## Active Skills

| Skill | Domain | Status | Purpose | Companion skills |
| --- | --- | --- | --- | --- |
| `agent-create-skill` | agent | active | Create or improve a skill from grounded friction by first running retrospective prompt analysis, then prompt optimization, then producing a portable `SKILL.md`. | `agent-retro-prompt`, `agent-prompt-master` |
| `agent-prompt-master` | agent | active | Refine raw user intent into a clearer, tool-targeted, execution-ready prompt while preserving the original request. | none |
...
| `dev-bdd-gherkin` | dev | active | Produce Gherkin-style behavior specifications and trap analysis while keeping domain behavior separate from implementation details. | `agent-prompt-master` |
| `dev-bdd-workflow` | dev | active | Define the reusable BDD delivery workflow: feature scope, constraints/traps, RED tests, minimal implementation, GREEN verification, refactor, decisions, quality, and documentation. | `flow-start-pipeline`, `dev-bdd-gherkin`, `dev-run-tests`, `dev-quality-check`, `quality-score` |
| `dev-delivery-review` | dev | active | Review a completed development delivery for architecture, tests, maintainability, docs readiness, and risk before final acceptance. | `dev-quality-check`, `dev-expert-code-review`, `quality-score` |
...
| `dev-github-issue-safe-fix` | dev | active | Resolve a GitHub issue by ID through a mandatory BDD-safe flow: issue verification, test-suite scan, prompt refinement, user-selected pipeline mode, stage 3/4/5 minimum, and no commit or push. | `agent-prompt-master`, `flow-start-pipeline`, `dev-bdd-workflow`, `dev-run-tests` |
...
| `flow-start-pipeline` | flow | active | Initialize a staged workflow from refined input, selected mode, language/domain hints, and requested stage set. | `agent-prompt-master`, `core-atomic-log`, `core-status` |
...
| `quality-score` | quality | active | Score outputs against explicit criteria and thresholds, identify blockers, and return a pass/fail/remediation verdict. | none |
