...
| `kb-memory-decay` | kb | active | Read-only health scan over memory notes (7 canonical checks: expired-stm, stale-mtm, ltm-low-confidence, review-overdue, superseded-candidate, promotion-eligible, orphaned-canonical-source). Dry-run only; never mutates. | `kb-memory-enrich`, `kb-obsidian-archive` | `fuhyo` |
| `kb-memory-enrich` | kb | active | Create or update dedicated Memory notes that wrap canonical zettels with operational recall metadata. Modeled on kb-obsidian-assemble's wrapper-note pattern. Enforces tier-promotion eligibility. Never modifies zettel bodies; never invents claims. | `kb-memory-recall`, `kb-obsidian-search`, `kb-obsidian-lint` | `fuhyo` |
| `kb-memory-recall` | kb | active | Task-aware memory recall: search dedicated memory notes by agent/project/intent, hydrate linked canonical zettels, return a bounded context_packet. Surfaces tier and scope promotion candidates. Read-only beyond mandatory recall counters. Falls back to kb-obsidian-search when no memory wrapper exists. Ōshō may invoke it only for pre-Kakugyō global planning recall; Fuhyō executes targeted per-agent recall steps planned by Kakugyō. | `kb-obsidian-search` | `osho`, `fuhyo` |
