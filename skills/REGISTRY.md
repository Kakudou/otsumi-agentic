...
| `kb-memory-recall` | kb | active | Task-aware memory recall: search dedicated memory notes by agent/project/intent, hydrate linked canonical zettels, return a bounded context_packet. Surfaces tier and scope promotion candidates. Read-only beyond mandatory recall counters. Falls back to kb-obsidian-search when no memory wrapper exists. | `kb-obsidian-search` | `fuhyo` |
