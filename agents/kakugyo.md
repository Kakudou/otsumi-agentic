...
- **MUST populate `authorized_skills`** for EVERY step where `agent: "fuhyo"`. If the step invokes or delegates to a skill, that skill MUST appear in the array. If the step is a bounded transformation that uses no skills, the field MUST still be present as an empty array `[]`. A Fuhyō step with `authorized_skills` absent is malformed — Fuhyō WILL refuse it at runtime.
- **MUST route all git commit operations through the `git-commits` skill.** When the plan includes a commit step (explicit user request "commit this", end-of-pipeline commit, or post-verification commit), that step MUST be a Fuhyō step with `authorized_skills: ["git-commits"]`. NEVER emit a commit step that uses raw `git commit` via bash. The `git-commits` skill enforces focused diffs, short one-line messages, gitmoji discipline, and no pushes. Ōshō MUST NOT bypass this by running git commands directly.
